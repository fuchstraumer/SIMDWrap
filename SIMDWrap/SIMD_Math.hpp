...
namespace simd {
	...
	static vec4 normalize(vec4 const& in) {
		vec4 mul = invsqrt(in);
		vec4 res = mul * in;
		return res;
	}
...
*/
	...
	// a * b + c
	static vec8 fma(vec8 const& a, vec8 const& b, vec8 const& c) {
		vec8 res;
	...
	// a * b - c
	static vec8 fms(vec8 const& a, vec8 const& b, vec8 const& c) {
		vec8 res;
	...
	static vec8 clamp(vec8 const& v0, vec8 const& min_val, vec8 const& max_val) {
	...
	static vec8 clamp(vec8 const& v0, const float& min, const float& max) {
